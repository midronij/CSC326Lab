...
<body>
	...
	<div class="nav">
		...
		<a href="/login"><button id="login" type="button" class="btn">Log In</button></a>
	</div>
	<div class="main">
...
def login():
    ...
    flow = flow_from_clientsecrets("client_secret.json", scope='https://www.googleapis.com/auth/plus.me https://www.googleapis.com/auth/userinfo.email', redirect_uri="/redirect")
...
<body>
    ...
    if user_logged_in:
        output+= '''<a href="/logout"><button id="login" type="button" class="btn">Log Out</button></a><h2 id="toptext">''' + str(request.get_cookie("email")) + '''</h2>'''
    else:
        output+= '''<a href="/login"><button id="login" type="button" class="btn">Log In</button></a>'''
