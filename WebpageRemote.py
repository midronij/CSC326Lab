...
def hello():
   ...
   output += '''<a href="''' + URL + '''/login"><button id="login" type="button" class="btn">Log In</button></a></section></html>'''
...
def login():
    ...
    flow = flow_from_clientsecrets("client_secret.json", scope='https://www.googleapis.com/auth/plus.me https://www.googleapis.com/auth/userinfo.email', redirect_uri=uri)
...
def redirect_page():
    ...
    flow = OAuth2WebServerFlow(client_id=CLIENT_ID, client_secret=CLIENT_SECRET, scope=SCOPE, redirect_uri=uri)
    ...
    output += '''<a href="''' + URL + '''/logout"><button id="logout" type="button" class="btn">Log Out</button></a>
	</section>
	</html>
'''
...
def displayResults():
        ...
        output = '''<a href="''' + URL + '''/login"><button id="login" type="button" class="btn">Log In</button></a>'''
        #results table
...
def displayResults():
        ...
        output = '''<h3>''' + user_email + '''</h3><br><a href="''' + URL + '''/logout"><button id="logout" type="button" class="btn">Log Out</button></a>'''
        global userHistory
...
#bottle.run(app=app)
run(host='0.0.0.0', port=80, debug=True) #note: localhost:8080 is hardcoded into login/logout buttons (change this later if possible)
